...
## 8. Module Phương pháp phân tích — handoff chi tiết (buổi 16)
...
| `/analytical-methods/new` | Tạo method + v1 Draft + workflow start/end |
| `/analytical-methods/[id]` | Overview metadata |
...
`AnalyticalMethod`, `MethodVersion`, `MethodDocument`, `MethodWorkflow`, `WorkflowNode`, `WorkflowEdge`, `MethodReagent`, `MethodEquipment`, `MethodQCRequirement`, `MethodAcceptanceCriteria`, `MethodSafetyNote`, `MethodApproval`, `MethodAIExtractionLog`, `MethodExecution`, `MethodExecutionStep`

FK cross-module: `MethodReagent` → `Chemical`/`Standard` · `MethodEquipment` → `Equipment` · `MethodSafetyNote` → `ChemicalReference` · safety check qua CAS → chem-info compatibility
...
| Components | `components/analytical-methods/*.tsx` · **`components/analytical-methods/workflow/*`** (11 files) |
...
|------------|-----------|-------|---------|
| `PP-ICP-WAT-001` | ICP-OES | 18 | Pb,Cd,As,Hg,Cr,Ni,Cu,Zn · link `EQ-ICP-001`, `CHEM-0030` |
| `PP-LCMS-PST-001` | LC-MS/MS | 20 | Acetamiprid QuEChERS · link `EQ-LC-MS-001`, `STD-0027` |
