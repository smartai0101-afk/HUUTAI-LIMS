...
- [ ] **CG01 flow:** CG01-001 → Reject → tạo mới Mã nhóm `CG01` → preview `CG01-002` hoặc `-003` · không lỗi *"Mã chuẩn pha chế đã tồn tại"*
...
- [ ] **Báo cáo:** `/preparation-history` — cột Mã nhóm + Mã lô · export Excel
...
Merge PR cursor/mobile-scroll-vercel-session → main · redeploy prod (`16d1efb`).
Hoặc QA browser prod: tab Truy xuất PSTD-0006 · dashboard · notification bell · §8 checklist.
...
- **Dev local:** http://localhost:3000 · `DATABASE_URL=file:./dev.db` · TG2 ghost → `TG2__deleted__86e5acb3` · bản TG2 mới (Nháp) sau QA
- **Git:** branch `cursor/mobile-scroll-vercel-session` · HEAD **`16d1efb`** · **đã push**
- **Prod:** https://huutai-lims-m929.vercel.app · deploy `4bbcb0b`+ · **chưa redeploy Phase 4** (`16d1efb`)

### Ưu tiên phiên sau

**Track A — Prod + merge**
1. Redeploy prod từ `16d1efb` · set `BLOB_READ_WRITE_TOKEN` + `CRON_SECRET` nếu thiếu
2. Merge PR `cursor/mobile-scroll-vercel-session` → `main`
3. QA prod checklist §8 (dashboard · reports · `/preparation-history` · notification bell)

**Track B — QA ISO còn lại (local/prod)**
1. Tab Truy xuất PSTD-0006 → PSTD-0005 → STD-0007 · reverse STD-0007
2. Amendment sau Approved (lý do bắt buộc)
3. Filter loại/trạng thái/ngày đầy đủ trên `/preparation-history`

**Track C — QA browser khác (§8)**
1. `/admin/people` · Import Excel 6 trang · sticky header · in tem nhãn TB
...
```
Đọc HANDOFF.md (§ Ghi chú phiên + § Lịch sử pha chế ISO) và PLAN-preparation-iso.md.
Redeploy prod từ 16d1efb và merge PR cursor/mobile-scroll-vercel-session → main.
Hoặc QA tab Truy xuất PSTD-0006 + checklist §8 prod.
Chỉ đọc file liên quan — không scan toàn repo.
```
...
### File tham chiếu (Inventory Transaction Engine — buổi 4 · uncommitted)
