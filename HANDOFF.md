...
> **Cập nhật:** 2026-06-26 (Nhân sự `/admin/people` uncommitted · Plan ISO lịch sử pha chế lưu tại `PLAN-preparation-iso.md` · **chưa code**)  
...
| **Quản lý nhân sự `/admin/people`** | 🟡 **Uncommitted local** — gộp User+Staff · FK `User.staffId` · migration `20260626_user_staff_link` · **cần commit** |
| **Lịch sử pha chế ISO 17025** | 🔴 **Chưa implement** — plan [`PLAN-preparation-iso.md`](PLAN-preparation-iso.md) · 5 phase todos pending |
...
### Phiên này (2026-06-26) — Plan LIMS hoàn tất ✅ (commit `d8adfa0` · chưa push/redeploy prod)
...
### Ưu tiên 0 — Commit Nhân sự `/admin/people` 🟡 (uncommitted local)
...
- [ ] **Commit** thay đổi (~15 file, trừ `prisma/prisma/`)
- [ ] **Local:** `npx prisma generate` (kill node nếu EPERM) · `npm run db:seed` sync `admin_people`

**Fix Prisma client stale:** `taskkill /F /IM node.exe` → `npx prisma generate` → xóa `.next` → `npm run dev`

### Ưu tiên 1 — Lịch sử pha chế ISO/IEC 17025 🔴 (plan · chưa code)
...
- [ ] Migration `20260627_preparation_iso`: workflowStatus, Staff FKs, deletedAt, `PreparationHistory`, `PreparationAuditLog`
- [ ] `lib/services/preparation-workflow.ts` + hook `prepared-chemicals.ts`, `prepared-standards.ts`, `modules.ts`
- [ ] `scripts/backfill-preparation-history.ts`

**Phase 2 — Workflow UI**
- [ ] FSM Draft→Prepared→Checked→Approved→Cancelled · amendment bắt buộc lý do
- [ ] 3 Prepared*Client: tabs Lịch sử / Truy xuất · soft delete
...
**File đọc khi bắt đầu:** `PLAN-preparation-iso.md` · `prisma/schema.prisma` (Prepared*) · `lib/actions/prepared-chemicals.ts` · `lib/actions/prepared-standards.ts` · `lib/audit.ts`

### Ưu tiên 0 — Prod ops + deploy `d8adfa0` 🟡

- [ ] **Push branch** `cursor/mobile-scroll-vercel-session` (commit `d8adfa0`)
...
- [ ] **Redeploy prod:** `npx vercel --prod --yes` từ `d8adfa0`
...
- [ ] **Excel import** round-trip 6 trang
...
Commit admin/people nếu chưa commit.
Triển khai Phase 1 plan ISO (schema + preparation-workflow + hook actions).
Hoặc Track A: push d8adfa0 + Turso migrate + redeploy prod.
...
- **Phiên kết thúc (2026-06-26):**
  - **Nhân sự:** Gộp Người dùng + Nhân viên → `/admin/people` · **uncommitted local** (chưa commit)
  - **ISO pha chế:** Phân tích + lập plan · lưu [`PLAN-preparation-iso.md`](PLAN-preparation-iso.md) · **chưa implement code**
  - **Fix runtime:** `/admin/people` PrismaClientValidationError → `prisma generate` + xóa `.next`
- **Dev local:** http://localhost:3000
- **Git:** branch `cursor/mobile-scroll-vercel-session` · HEAD `ac3102b` (HANDOFF) · **`d8adfa0` + admin/people uncommitted**
- **Prod:** https://huutai-lims-m929.vercel.app — deploy cũ · chưa có `d8adfa0` + admin/people + ISO pha chế

### Ưu tiên phiên sau (chọn 1 track hoặc song song)

**Track A — Commit + prod (ngắn)**
1. Commit admin/people + push
2. Turso migrations (`notifications`, `user_staff_link`) + prod redeploy
3. QA prod checklist §8

**Track B — ISO lịch sử pha chế (lớn)**
1. Đọc `PLAN-preparation-iso.md` + `HANDOFF.md` § Lịch sử pha chế ISO
2. Phase 1: migration + `preparation-workflow.ts`
3. Phase 2–4 theo plan

### Recommended Next Prompt

```
Đọc HANDOFF.md (§ Ghi chú phiên + § Lịch sử pha chế ISO) và PLAN-preparation-iso.md.
Nếu chưa commit: commit admin/people trước.
Triển khai Phase 1 plan ISO (schema + preparation-workflow + hook actions).
Chỉ đọc file liên quan — không scan toàn repo.
```

### File tham chiếu (Plan ISO lịch sử pha chế — pending)

| Mục đích | File |
|----------|------|
| **Plan đầy đủ** | [`PLAN-preparation-iso.md`](PLAN-preparation-iso.md) |
| Schema hiện tại | `prisma/schema.prisma` — PreparedChemical, PreparedStandard, PreparedStrain, ingredient models |
| Actions pha chế | `lib/actions/prepared-chemicals.ts`, `prepared-standards.ts`, `modules.ts` (PreparedStrain) |
...
| UI hiện tại | `components/prepared-chemicals/*`, `prepared-standards/*`, `prepared-strains/*` |

### File tham chiếu (Nhân sự `/admin/people` — uncommitted)
