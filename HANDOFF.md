...
> **Cập nhật:** 2026-06-26 (Commit `d8adfa0` — Plan LIMS: dashboard/báo cáo thật · Staff · Sổ cái · Cảnh báo cron · hardening)  
...
| **Authentication & RBAC** | ✅ Login/logout · JWT session · middleware · **25 quyền sidebar** (`admin_staff`, `inventory_ledger` mới) · Admin UI |
...
| **Quản lý nhân viên `/admin/staff`** | ✅ Commit `d8adfa0` — CRUD `Staff` · permission `admin_staff` (Admin/LM) |
...
**Menu Quản trị:** Người dùng · Phân quyền · Giao việc · **Nhân viên**.  
...
| **Chưa UI InventoryTransaction / Staff** | 🟢 | `/inventory-ledger` + `/admin/staff` (`d8adfa0`) |
...
| **Seed local — quyền mới** | 🟡 | Chạy `npm run db:seed` nếu thiếu permission `inventory_ledger`, `admin_staff` |
...
| Thành phần | Chi tiết |
|------------|----------|
| **Staff `/admin/staff`** | [`AdminStaffClient.tsx`](components/admin/AdminStaffClient.tsx) · permission `admin_staff` |
...
- RBAC **25 quyền** (+ `inventory_ledger`, `admin_staff`)
...
| Staff | `components/admin/AdminStaffClient.tsx`, `lib/actions/admin-staff.ts` |
...
Push branch `cursor/mobile-scroll-vercel-session` (commit d8adfa0) nếu chưa push.
Turso prod: migration notifications + backfill specs TB.
Vercel env: BLOB_READ_WRITE_TOKEN, CRON_SECRET → redeploy prod (npx vercel --prod).
QA prod: dashboard, reports, ledger, staff, usage lot picker, equipment labels, notifications.
...
| **Quản trị** | `admin-nav-open` | 4 — Người dùng · Phân quyền · Giao việc · **Nhân viên** |
...
- **Phiên kết thúc (2026-06-26):** Hoàn tất plan LIMS — dashboard/báo cáo thật · Staff · Sổ cái · lot picker · quantity guard · cron cảnh báo · session guard · import P1 · commit **`d8adfa0`**
- **Dev local:** http://localhost:3000
- **Git:** branch `cursor/mobile-scroll-vercel-session` · HEAD **`d8adfa0`** · **chưa push** · working tree sạch (trừ `prisma/prisma/` — không commit)
- **Prod:** https://huutai-lims-m929.vercel.app — deploy cũ `dpl_hA5ZQf4JWL7Bq7uo9zFGRVZ4dJpH` · **chưa có `d8adfa0`**
- **Ưu tiên phiên sau:**
  1. **Push** `d8adfa0` lên origin
  2. **Turso:** migration `20260626_notifications` + backfill specs TB
  3. **Vercel env:** `BLOB_READ_WRITE_TOKEN`, `CRON_SECRET` → **redeploy prod**
  4. **QA prod:** dashboard, reports, ledger, staff, lot picker, in tem nhãn, notifications, sticky header, Excel import
  5. **Local:** `npm run db:seed` nếu thiếu quyền `inventory_ledger`, `admin_staff`

### File đọc trước (phiên sau — prod + QA)
...
| Staff | `components/admin/AdminStaffClient.tsx`, `lib/actions/admin-staff.ts` |
| Ledger | `lib/services/inventory-ledger.ts`, `components/inventory-ledger/InventoryLedgerClient.tsx` |
| Cron alerts | `app/api/cron/proactive-alerts/route.ts`, `vercel.json` |
| Permissions | `lib/auth/nav-permissions.ts`, `lib/auth/permissions.ts` |
...
```
Đọc HANDOFF.md (§ Pending Tasks + Ghi chú phiên).
Push branch (commit d8adfa0) nếu chưa push.
Turso prod: migration notifications + backfill specs TB.
Vercel env: BLOB_READ_WRITE_TOKEN, CRON_SECRET → redeploy prod.
QA prod: dashboard, reports, ledger, staff, usage lot picker, equipment labels, notifications.
Chỉ đọc file liên quan — không scan toàn repo.
```
...
| Staff | `app/admin/staff/page.tsx`, `components/admin/AdminStaffClient.tsx`, `lib/actions/admin-staff.ts` |
