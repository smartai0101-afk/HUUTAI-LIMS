...
> **Cập nhật:** 2026-06-26 (In tem nhãn TB · redeploy prod `dpl_hA5ZQf4JWL7Bq7uo9zFGRVZ4dJpH` · uncommitted local)  
...
| **Desktop scroll bảng** | ✅ Code uncommitted — `table-scroll-viewport` · cuộn X+Y trong bảng · **chỉ sticky header** (`lg+`) · **không pin cột** · QA browser pending |
...
| **Deploy Vercel** | ✅ Redeploy 2026-06-26 — `dpl_hA5ZQf4JWL7Bq7uo9zFGRVZ4dJpH` → https://huutai-lims-m929.vercel.app · **deploy từ working tree uncommitted** (HEAD vẫn `ed32b5b`) · **QA login prod pending** |
...
| **Danh mục TB — In tem nhãn** | ✅ Code uncommitted — checkbox chọn · preview modal · in A4 grid · profile ladder 7×5–7×6cm · logo căn giữa · **`public/sci-tech-logo.png`** · **đã redeploy prod** · **QA browser + commit pending** |
...
| **Vercel (prod)** | https://huutai-lims-m929.vercel.app — redeploy 2026-06-26 (`dpl_hA5ZQf4JWL7Bq7uo9zFGRVZ4dJpH`) · gồm Notification + specs TB + in tem nhãn + sticky header (uncommitted local) |
...
| **Import Excel — tối ưu P1 (batch query)** | 🟡 | File lớn vẫn chậm · xem § Pending Tasks P1 |
| **Desktop sticky header + viewport** | 🟡 | Code xong · **chưa commit** (~9 file) · QA browser pending |
| **Chưa commit sticky header** | 🟡 | `DataTable.tsx`, `globals.css`, catalog reorder clients · **không** gồm Notification/specs TB (đã commit `ed32b5b`) |
| **Turso chưa migration Notification** | 🟡 | Prod cần `db execute` migration `20260626_notifications` trước deploy `ed32b5b` |
| **Turso/prod chưa backfill specs TB** | 🟡 | Chạy `npx tsx scripts/backfill-equipment-specifications.ts` trên prod sau redeploy |
| **Seed local chưa chạy** | 🟡 | Prod Turso đã seed · local SQLite: `npm run db:seed` nếu cần demo data |

---

## 2. Completed Work

### Phiên này (2026-06-26) — In tem nhãn thiết bị + Redeploy prod ✅ (code uncommitted · redeployed)

#### In tem nhãn — Danh mục thiết bị (`/equipment/catalog`) ✅ (code · QA browser + commit pending)
...
#### Redeploy Vercel prod ✅ (2026-06-26)

- **Project:** `huutai/huutai-lims-m929`
- **Deployment:** `dpl_hA5ZQf4JWL7Bq7uo9zFGRVZ4dJpH`
- **URL:** https://huutai-lims-m929.vercel.app
- **Nguồn:** working tree local **uncommitted** (HEAD git vẫn `ed32b5b`)
- **Gồm:** Notification (`ed32b5b`) + sticky header + in tem nhãn TB + specs column UI
...
#### Bảng desktop — scroll viewport + chỉ cố định tiêu đề ✅ (code · chưa commit · QA pending)
...
| **File sửa (uncommitted)** | `DataTable.tsx`, `globals.css`, catalog clients (reorder `storageLocation`), `HANDOFF.md` |
...
| Chưa commit sticky header + viewport | 🟡 | ~10 file · HEAD `7e5ff9f` |
...
- [x] **Redeploy Vercel** 2026-06-26 (`dpl_hA5ZQf4JWL7Bq7uo9zFGRVZ4dJpH`) — **Turso migrate vẫn cần trước khi bell hoạt động prod**
...
### Ưu tiên 0 — In tem nhãn thiết bị ✅ (code · redeployed · QA + commit pending)
...
- [x] **Redeploy prod** 2026-06-26 (cùng deployment in tem nhãn)
...
- [ ] **Commit** in tem nhãn + sticky header + các thay đổi uncommitted còn lại
...
### Ưu tiên 0 — Sticky header + scroll viewport (bỏ pin cột) ✅ (code · chưa commit · QA pending)
...
- [ ] **Commit** sticky header changes (~10 file)
...
- [ ] **P1 — Giảm query:** Batch preload master + `StockLot` trong `importCatalogLotRows` / `previewCatalogImport`
...
```
Đọc HANDOFF.md (§ Pending Tasks + Ghi chú phiên).
Turso prod: apply migration 20260626_notifications + backfill specs TB (nếu chưa).
QA prod: https://huutai-lims-m929.vercel.app — login · in tem nhãn /equipment/catalog · notification bell.
Nếu QA OK → commit uncommitted (~15 file).
...
- **Không** revert: … **In tem nhãn thiết bị**, **Notification system** (`ed32b5b`), **Cột Thông số kỹ thuật TB**, **Trang Tài khoản**, **dev DB routing**, **Vercel Blob** (`lib/file-storage.ts`), **Export/Import Excel catalog** (6 trang), **Fix import P0**, **Equipment layout shell** — trừ khi user yêu cầu
...
- **Phiên kết thúc (2026-06-26):** In tem nhãn TB (profile ladder + logo căn giữa) · **redeploy prod** `dpl_hA5ZQf4JWL7Bq7uo9zFGRVZ4dJpH` · **chưa commit** (HEAD `ed32b5b`)
- **Dev local:** http://localhost:3000
- **Git:** branch `cursor/mobile-scroll-vercel-session` · HEAD `ed32b5b` (pushed) · **working tree có thay đổi chưa commit**
- **Uncommitted (~15 file):** in tem nhãn (`EquipmentLabel*`, `equipment-label-print.*`, `lib/equipment-label-print.ts`, `sci-tech-logo.png`, `EquipmentCatalogClient`, `EquipmentModuleShell`) + sticky header (`DataTable.tsx`, `globals.css`, catalog clients) + `HANDOFF.md`
- **Prod:** https://huutai-lims-m929.vercel.app — **đã redeploy** working tree local 2026-06-26
- **Ưu tiên phiên sau:**
  1. **Turso:** migration `20260626_notifications` + backfill specs TB (nếu chưa)
  2. **QA prod:** login · bell notification · `/equipment/catalog` in tem nhãn (EQ-AUT-001, EQ-BAL-001)
  3. **Commit** toàn bộ uncommitted (in tem nhãn + sticky header) sau QA OK
  4. QA Excel import 6 trang · Blob prod · sticky header desktop/mobile

### File đọc trước (phiên sau — QA + commit)

| Mục đích | File |
|----------|------|
| Bàn giao | `HANDOFF.md` § Pending Tasks + § Ghi chú phiên |
| In tem nhãn | `lib/equipment-label-print.ts`, `components/equipment/EquipmentLabel*.tsx`, `equipment-label-print.css` |
| Notification | `lib/notifications/*`, `app/api/notifications/*` |
| Turso migrate | `prisma/migrations/20260626_notifications/migration.sql` |
...
### File tham chiếu (In tem nhãn thiết bị — mới uncommitted)

| Mục đích | File |
|----------|------|
| Mapper + profiles | `lib/equipment-label-print.ts` |
| Tem component | `components/equipment/EquipmentLabelPrint.tsx` |
| CSS + print | `components/equipment/equipment-label-print.css` |
| Preview modal | `components/equipment/EquipmentLabelPreviewModal.tsx` |
| Catalog wiring | `components/equipment/EquipmentCatalogClient.tsx`, `EquipmentModuleShell.tsx` |
| Logo | `public/sci-tech-logo.png` |
