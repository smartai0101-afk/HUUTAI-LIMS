...
> **Cập nhật:** 2026-06-27 (ISO pha chế Phase 1+2 commit `0d9518f` · Nhân sự commit `d04f674` · **chưa push prod**)  
...
| **`/prepared-chemicals`, `/prepared-standards`, `/prepared-strains`** | ✅ Lot picker · trừ/hoàn theo lot · **ISO workflow** Draft→Prepared→Checked→Approved (`0d9518f`) · tab Lịch sử · **QA browser pending** |
...
| **Authentication & RBAC** | ✅ Login/logout · JWT session · middleware · **24 quyền sidebar** (`admin_people` gộp users+staff) · Admin UI |
...
| **Deploy Vercel** | 🟡 Commit `d8adfa0` **chưa push/redeploy prod** · prod hiện tại vẫn deploy cũ (`dpl_hA5ZQf4JWL7Bq7uo9zFGRVZ4dJpH`) · **cần push + `npx vercel --prod`** |
...
| **Quản lý nhân sự `/admin/people`** | ✅ Commit `d04f674` — gộp User+Staff · FK `User.staffId` · migration `20260626_user_staff_link` · **chưa push prod** |
| **Lịch sử pha chế ISO 17025** | 🟡 **Phase 1+2 done** (`0d9518f`) · Phase 3–4 pending · plan [`PLAN-preparation-iso.md`](PLAN-preparation-iso.md) |
...
### Phiên này (2026-06-27) — Nhân sự + ISO pha chế Phase 1+2 ✅
...
| **2 Clients** | 3 Prepared*Client + `ModuleCrudClient` | Filter quy trình · cột Quy trình · tabs Chi tiết/Lịch sử/Truy xuất (Truy xuất = placeholder Phase 3) |
...
**Turso prod:** chưa apply migration `20260627_preparation_iso` · chưa backfill.

**Lệnh kiểm tra:** `npx tsc --noEmit` pass

**Git:** branch `cursor/mobile-scroll-vercel-session` · HEAD `0d9518f` · **chưa push**
...
- [x] **Commit** `d04f674`
- [ ] **Turso prod:** apply migration `20260626_user_staff_link`
- [ ] **Local:** `npm run db:seed` nếu thiếu permission `admin_people`

### Ưu tiên 1 — Lịch sử pha chế ISO/IEC 17025 🟡 (Phase 1+2 done · Phase 3–4 pending)
...
- [x] `scripts/backfill-preparation-history.ts` (local đã chạy)
- [ ] **Turso prod:** apply migration + backfill

**Phase 2 — Workflow UI** ✅ (`0d9518f`)
- [x] FSM Draft→Prepared→Checked→Approved→Cancelled · amendment bắt buộc lý do
- [x] 3 Prepared*Client: tabs Lịch sử / Truy xuất (placeholder) · soft delete · Staff picker
- [ ] **QA browser:** tạo Draft → Prepared (trừ tồn) → Checked → Approved · amend Approved · cancel · xóa mềm

**Phase 3 — Traceability** 🔴 (chưa làm)
- [ ] `preparation-traceability.ts` · cây PSTD-0006→PSTD-0005→STD-0007
- [ ] Reverse lookup trên catalog gốc · `/preparation/[type]/[id]`
- [ ] Tab **Truy xuất** thay placeholder

**Phase 4 — Báo cáo** 🔴 (chưa làm)
- [ ] `/preparation-history` · export Excel 14 cột · nav `preparation_history`
- [ ] Cập nhật `prepared-excel.ts` / import · seed chain trong `links.ts`

**File đọc khi Phase 3:** `PLAN-preparation-iso.md` · `lib/services/preparation-workflow.ts` · `prisma/seed-data/links.ts` · `components/preparation/PreparationDrawerTabContent.tsx`

**Turso prod (một lần):**

```powershell
npx prisma db execute --file prisma/migrations/20260626_user_staff_link/migration.sql --schema prisma/schema.prisma
...
# với Turso prod:
# $env:NODE_ENV="production"; npx tsx scripts/backfill-preparation-history.ts
```

### Ưu tiên 0 — Prod ops + deploy 🟡

- [ ] **Push branch** `cursor/mobile-scroll-vercel-session` (HEAD `0d9518f` — gồm `d8adfa0`, `d04f674`, ISO)
- [ ] **Turso prod:** apply migration `20260626_notifications`
- [ ] **Turso prod:** apply `20260626_user_staff_link` + `20260627_preparation_iso` + backfill preparation history
- [ ] **Turso prod:** backfill specs TB (`NODE_ENV=production`)
- [ ] **Vercel env:** `BLOB_READ_WRITE_TOKEN`, `CRON_SECRET` (cho cron cảnh báo)
- [ ] **Redeploy prod:** `npx vercel --prod --yes`
- [ ] **Local seed** nếu thiếu quyền mới: `npm run db:seed`
...
- [ ] **`/prepared-chemicals`** — workflow Draft→Approved · tab Lịch sử · amend · xóa mềm
...
- **Phiên kết thúc (2026-06-27):**
  - **Nhân sự:** Commit `d04f674` — `/admin/people` gộp User+Staff · migration `20260626_user_staff_link`
  - **ISO pha chế Phase 1:** Schema `20260627_preparation_iso` · `preparation-workflow.ts` · hooks actions · backfill script (local 12 bản ghi)
  - **ISO pha chế Phase 2:** FSM UI · amendment dialog · soft delete · Staff picker · tabs Lịch sử/Truy xuất · commit `0d9518f`
  - **Kiểm tra:** `npx tsc --noEmit` pass
- **Dev local:** http://localhost:3000 · migration ISO đã apply local · `prisma generate` OK
- **Git:** branch `cursor/mobile-scroll-vercel-session` · HEAD **`0d9518f`** · **chưa push**
- **Prod:** https://huutai-lims-m929.vercel.app — deploy cũ · chưa có `d8adfa0`/`d04f674`/`0d9518f`

### Ưu tiên phiên sau (chọn 1 track hoặc song song)

**Track A — Push + prod (ngắn)**
1. Push branch `cursor/mobile-scroll-vercel-session`
2. Turso: `notifications` + `user_staff_link` + `preparation_iso` + backfill preparation history
3. Redeploy + QA prod checklist §8 (kèm workflow pha chế)

**Track B — ISO Phase 3 Traceability (lớn)**
1. Đọc `PLAN-preparation-iso.md` Phase 3 + `prisma/seed-data/links.ts`
2. `lib/services/preparation-traceability.ts` — forward/reverse tree
3. Tab Truy xuất thay placeholder · reverse lookup trên catalog gốc · route `/preparation/[type]/[id]`

**Track C — ISO Phase 4 Báo cáo**
1. `/preparation-history` + export Excel 14 cột · nav permission `preparation_history`

### Recommended Next Prompt

```
Đọc HANDOFF.md (§ Ghi chú phiên + § Lịch sử pha chế ISO) và PLAN-preparation-iso.md.
Triển khai Phase 3 ISO (traceability tree + tab Truy xuất + reverse lookup).
Hoặc Track A: push + Turso migrate + redeploy prod.
Chỉ đọc file liên quan — không scan toàn repo.
```

### File tham chiếu (ISO pha chế — Phase 1+2 done · Phase 3 pending)
...
| History | `lib/services/preparation-history.ts`, `scripts/backfill-preparation-history.ts` |
| Labels | `lib/preparation-workflow-labels.ts`, `lib/map-preparation-workflow.ts` |
| UI shared | `components/preparation/*` |
| Clients | `PreparedChemicalsClient`, `PreparedStandardsClient`, `PreparedStrainsClient`, `ModuleCrudClient` |
