...
export const PREPARED_CHEMICAL_IMPORT_COLUMN_MAP: Record<string, string> = Object.fromEntries(
  PREPARED_CHEMICAL_CSV_FIELD_KEYS.map((header) => {
    const keyMap: Record<string, string> = {
      ...
      "Công thức": "formula",
      "Nồng độ lý thuyết": "concentration",
      "Nồng độ": "concentration",
      "Đơn vị nồng độ": "concentrationUnit",
      "Nồng độ thực tế": "finalConcentration",
      ...
      "Trạng thái": "status",
      "Trạng thái quy trình": "workflowStatusLabel",
      "Ghi chú": "notes",
...
export function buildPreparedStrainExportRows(
  ...
  return items.map((item) => ({
    ...
    "Cấp chủng": String(item.level ?? ""),
    "Nồng độ lý thuyết": String(item.concentration ?? ""),
    "Nồng độ thực tế": String(item.finalConcentration ?? ""),
    "Lot": String(item.lot ?? ""),
    ...
    "Trạng thái quy trình": String(item.workflowStatusLabel ?? ""),
