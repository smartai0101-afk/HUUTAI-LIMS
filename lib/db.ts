...
const MODEL_DELEGATES = new Set(
  Prisma.dmmf.datamodel.models.map((model) => modelKey(model.name)),
);
...
function delegateError(modelName: string): Error {
  return new Error(
    `Prisma client thiếu model "${modelName}". Chạy: npx prisma generate → tắt dev server → xóa .next → npm run dev`,
  );
}
