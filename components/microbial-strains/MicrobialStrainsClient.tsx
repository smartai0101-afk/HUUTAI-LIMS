...
type StrainLotRow = MicrobialStrainView & CatalogLotRowMeta;
...
const tailFormFields: { key: keyof FormState; label: string }[] = [
  { key: "storageLocation", label: "Vị trí lưu" },
];

const detailFields: { key: keyof MicrobialStrainView; label: string }[] = [
  { key: "code", label: "Mã" },
  { key: "name", label: "Tên chủng" },
  { key: "strainGroup", label: "Nhóm chủng" },
  { key: "manufacturer", label: "Hãng/Nguồn cung cấp" },
  { key: "atccProductCode", label: "Mã ATCC / PRODUCT CODE" },
  { key: "lot", label: "Lot Number" },
  { key: "purity", label: "Purity" },
...
export function MicrobialStrainsClient({ items, groupOptions }: { items: MicrobialStrainView[]; groupOptions: string[] }) {
  ...
  const openEdit = (item: MicrobialStrainView) => {
    setIsEditing(true);
  ...
  const handleSubmit = () => {
    ...
    if (!form.expiryDate.trim()) {
      addToast("Ngày hết hạn là bắt buộc", "error");
      return;
    }
    ...
    startTransition(async () => {
      const result = isEditing ? await updateMicrobialStrain(fd) : await createMicrobialStrain(fd);
      if (result.error) {
        addToast(result.error, "error");
        return;
      }
      addToast(isEditing ? "Đã cập nhật chủng" : "Đã thêm chủng mới", "success");
      setIsFormOpen(false);
      setCoaFile(null);
      router.refresh();
    });
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      ...
      if (target.stockLotId) {
        ...
        fd.set("id", target.id);
        result = await deleteMicrobialStrain(fd);
      }
      if (result.error) {
        addToast(result.error, "error");
        return;
      ...
      addToast(
        target.stockLotId ? `Đã xóa lot ${target.lot}` : "Đã xóa chủng thành công",
        "success",
      );
    ...
    <AppShell>
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-slate-500">Microbial stock</p>
            <h1 className="text-2xl font-semibold text-slate-900">Chủng gốc vi sinh</h1>
          </div>
          ...
          tabContent={
            selected ? (
              <div className="grid gap-3 sm:grid-cols-2">
                {detailFields.map((f) => (
                  <div key={f.key} className={f.key === "storageCondition" ? "sm:col-span-2" : ""}>
        ...
        <ModalShell open={isFormOpen} onClose={() => setIsFormOpen(false)} className="max-w-3xl rounded-2xl bg-white p-6 shadow-2xl">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold">{isEditing ? "Sửa chủng" : "Thêm chủng"}</h2>
                <button type="button" onClick={() => setIsFormOpen(false)}><X className="h-5 w-5 text-slate-500" /></button>
              </div>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-sm text-slate-600">Nhóm chủng</label>
                  <input
                    list="strain-group-options"
                    value={form.strainGroup}
                    onChange={(e) => setForm((p) => ({ ...p, strainGroup: e.target.value }))}
                    placeholder="Vi khuẩn, Nấm men hoặc nhóm mới"
                    className="h-11 w-full rounded-xl border border-slate-200 px-3 text-sm"
                  />
                  <datalist id="strain-group-options">
                ...
                {tailFormFields.map((f) => (
                  <div key={f.key}>
                    <label className="mb-1 block text-sm text-slate-600">{f.label}</label>
                    <input value={form[f.key]} onChange={(e) => setForm((p) => ({ ...p, [f.key]: e.target.value }))} className="h-11 w-full rounded-xl border border-slate-200 px-3 text-sm" />
                  </div>
                ))}
              </div>
        ...
        <ConfirmDialog
          open={!!deleteTarget}
          title={deleteTarget?.stockLotId ? "Xóa lot tồn kho" : "Xóa chủng"}
          message={
            deleteTarget?.stockLotId
              ? `Bạn có chắc muốn xóa lot "${deleteTarget.lot}" của ${deleteTarget.name} (${deleteTarget.code})? Chủng gốc và các lot khác vẫn được giữ.`
